...
<div region="west" style="width: 200px;" title="导航菜单" split="true">
	<div class="easyui-accordion" data-options="fit:true,border:false">
		<c:forEach items="${list }" var="privilege">
			<div title="${privilege.name }"  data-options="iconCls:'${privilege.icon }'" style="padding:10px">
				<c:forEach items="${privilege.children }" var="child">
					<a href="javascript:openTab('${child.name}','${child.url}','${child.icon}')" 
					class="easyui-linkbutton" data-options="plain:true,iconCls:'${child.icon }'" 
					style="width: 150px;">${child.name }</a>
				</c:forEach>
			</div>
		</c:forEach>
